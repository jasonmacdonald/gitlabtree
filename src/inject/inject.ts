...
class GitLabTree {
  ...
  constructor() {
	...
	files.id = "gitlab-tree-plugin-top"
    // Obtain metadata
    ...
    this.strippedFileNames = this.removePathPrefix(
      this.fileNames,
      this.pathPrefix
    );
    ...
    const fileNamesDOM: HTMLDivElement = this.convertFolderStructureToDOM(
      this.pathPrefix,
      this.createFolderStructure(this.strippedFileNames)
    );
    ...
    this.expandListener = (e: MouseEvent) =>
      (e.target as HTMLElement).classList.contains("holder")
        ? this.toggleExpand(e)
        : undefined;
  ...
  obtainMetadata(): IMetadata[] {
    const metadataFiles_v10_3_and_latest: () => HTMLElement[] = () =>
      Array.prototype.slice.call(
        document.querySelectorAll(
          ".diff-file-changes .dropdown-content li:not(.hidden)"
        )
      );
    const metadataFiles_v9_5: () => HTMLElement[] = () =>
      Array.prototype.slice.call(document.querySelectorAll(".file-stats li"));
  ...
  obtainMetadata_latest(rawFilesMetadata: HTMLElement[]) {
    ...
    for (let rawFileMetadata of rawFilesMetadata) {
      const svgElement: HTMLElement = rawFileMetadata.querySelector(
        "svg.diff-file-changed-icon"
      ) as HTMLElement;
      const typeRaw: string = svgElement
        .querySelector("use")
        .getAttribute("xlink:href")
        .split("#")[1];
      const hash: string = rawFileMetadata
        .querySelector("a")
        .getAttribute("href");
      const filename: string = rawFileMetadata
        .querySelector(".diff-changed-file")
        .getAttribute("title");
  ...
  obtainMetadata_v10_3(rawFilesMetadata: HTMLElement[]) {
    ...
    for (let rawFileMetadata of rawFilesMetadata) {
      const classList = rawFileMetadata.querySelector("a i:first-child")
        .classList;
      const hash: string = rawFileMetadata
        .querySelector("a")
        .getAttribute("href");
      let filename: string = rawFileMetadata
        .querySelector(".diff-file-changes-path")
        .textContent.trim();
      let type: EFileState = EFileState.UPDATED;
  ...
  obtainMetadata_v9_5(rawFilesMetadata) {
    ...
    for (let rawFileMetadata of rawFilesMetadata) {
      const typeRaw: string[] = Array.prototype.slice.call(
        rawFileMetadata.querySelector("span:first-child").classList
      );
      const hash: string = rawFileMetadata
        .querySelector("a")
        .getAttribute("href");
  ...
  createFolderStructure(fileNames: string[]): any {
    ...
    return this.collapse(structure);
  }

  collapse(obj, dst = {}, prefix="") {
    let val, isObj;
	...
	if(prefix){
		...
		if ((keys.length == 1 && typeof obj[keys[0]] != "object") || keys.length > 1) {
        ...
        if (typeof entry === "number") {
          ...
          if (metadata.commented) {
            ...
            commentElement.classList.add(
              "fa",
              "fa-comments-o",
              CSS_PREFIX + "-file-commented-icon"
            );
  ...
  toggleExpand(event: MouseEvent) {
    ...
    let isMainFolder =
      document.querySelector(`.${CSS_PREFIX}-left > .folder`) === folder;

    if (!isMainFolder) {
      folder.classList.remove(
        CSS_PREFIX + "-folder-collapsed",
        CSS_PREFIX + "-folder-expanded"
      );
      folder.classList.add(
        CSS_PREFIX + (isExpanded ? "-folder-collapsed" : "-folder-expanded")
      );
    }
  }
  ...
  hashChanged(): void {
	...
	if(newHash === "#gitlab-tree-plugin-top") return;
  ...
  showFile(hash?: string): void {
    ...
    if (this.lastActive) {
      this.getFileHolderByHash(this.lastActive).classList.add(
        CSS_PREFIX + "-hidden"
      );
      this.getFileLinkByHash(this.lastActive).classList.remove(
        CSS_PREFIX + "-file-active"
      );
    }

    hash =
      this.metadata.filter(m => m.hash === hash).length > 0
        ? hash
        : this.metadata[0].hash; // if hash is invalid use default hash
	...
	location.hash = "gitlab-tree-plugin-top";
  }

  getFileHolderByHash(hash: string): HTMLElement {
    return this.rightElement.querySelector(
      `[id='${hash.substr(1)}']`
    ) as HTMLElement;
  }
